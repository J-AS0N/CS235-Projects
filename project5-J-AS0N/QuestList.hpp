...
10/29/23
Project 5
This is the interface implementation for QuestList
...
struct Quest {
    std::string title_;  // quest_title_
    std::string description_;  // quest_description_
    bool completed_;  // quest_complete_
    int experience_points_;  // exp_points_
    ...
    Quest(const std::string& title, const std::string& descript, bool completion, int xp, const std::vector<Quest*>& dependencies, const std::vector<Quest*>& subquests):
    title_{title}, description_{descript}, completed_{completion}, experience_points_{xp} {
...
*/
        QuestList(const std::string& file_name);  // parameterized constructor
...
*/
        bool addQuest(const std::string& quest_title, const std::string& quest_description, const bool& complete, const int& exp, const std::vector<Quest*>& dependency, const std::vector<Quest*>& subquest);

    /**
    @param:  A Quest pointer
    @return: A boolean indicating if all the given quest's dependencies are completed
*/
        bool dependenciesComplete(Quest* dependencies_complete);
...
*/
        ...
        // helper functions
        void queryHelp(Quest* query);
        void quest_detail_helper(Quest* q, int num_of_spaces);
};
