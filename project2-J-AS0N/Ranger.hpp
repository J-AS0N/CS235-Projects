...
#ifndef RANGER_HPP // guards ~ prevents re-definition of the same functions
#define RANGER_HPP // Capitalize the letters (for some reason :/ )
...
*/
        Ranger(const std::string& name, const std::string& race, int vitality = 0, int armor = 0, int level = 0, bool enemy = false, const std::vector<Arrows>& arrow = {}, const std::vector<std::string>& affinity = {}, bool companion = false); //WIP);  // parameterized constructor
