...
*/
Ranger::Ranger() {  // default initializes all private members
    Character();
    arrows_ = {};
    affinities_ = {};
...
{
    for (int i = 0; i < arrow.size(); i++) {  // loops through the arrow vector
        addArrows(arrow[i].type_, arrow[i].quantity_);  // calls the addArrows function
    }

    for (int i = 0; i < affinity.size(); i++) {  // loops through the affinity vector
        addAffinity(affinity[i]);  // calls the addAffinity function
    }
...
**/
void Ranger::setCompanion(const bool& companion) {  // set has_companion_ to the user's input
...
bool Ranger::fireArrow(const std::string& arrow) {
    std::string new_arrow1 = "";  // empty string
    for (int i = 0; i < arrow.length(); i++) {  // loops through the string
        if (isalpha(arrow[i])) {  // if each element is in the alphabet
            new_arrow1 += char(toupper(arrow[i]));  // convers it to uppercase
        }
    }

    for (int i = 0; i < arrows_.size(); i++) {  // loops through the arrows_ vector
        if (arrows_[i].type_ == new_arrow1 && arrows_[i].quantity_ > 0) { // if the i th element in the vector is equal to the user's input AND the quantity is greater than 0
            arrows_[i].quantity_--;  // subtract the quantity by 1
            return true;  // return true
        }
    }
    return false;
...
**/
std::vector<Arrows> Ranger::getArrows() const {  // return arrows_
    return arrows_;
}
...
**/
std::vector<std::string> Ranger::getAffinities() const {  // return affinities_
    return affinities_;
}
...
**/
bool Ranger::getCompanion() const{  // return has_companion_
