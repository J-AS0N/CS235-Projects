...
*/
template<class ItemType>  // need to have template<class ItemType> above EVERY function
void ArrayBag<ItemType>::operator/= (const ArrayBag<ItemType>& array_one) {
   for (int i = 0; i < getCurrentSize(); i++) {
      if (getCurrentSize() == DEFAULT_CAPACITY) {  // stops the loop if getCurrentSize is equal to the DEFAULT_CAPACITY -> meaning: there's no more space to add
         break;
      }
      if (!contains(array_one.items_[i])) {  // if the element is not contained in the original array
         this -> add(array_one.items_[i]);  // add it to the array
      }
   }
}
...
*/
template<class ItemType>
void ArrayBag<ItemType>::operator+= (const ArrayBag<ItemType>& array_two) {
   for (int i = 0; i < getCurrentSize(); i++) {
      if (getCurrentSize() == DEFAULT_CAPACITY) {  // breaks if getCurrentSize is equal to the DEFAULT_CAPACITY
         break;
      }
      this -> add(array_two.items_[i]);  // adds the element into the array (includes the duplicates)
      }
}
