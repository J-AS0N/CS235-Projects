...
*/
bool Character::operator== (const Character& new_character) const {  // checks if the two characters are equal to each other
    if (name_ == new_character.name_ && race_ == new_character.race_ && level_ == new_character.level_ && enemy_ == new_character.isEnemy()) {
        return true;
    }
    return false;
}
...
/**
  ...
  @return     : Returns true if the right hand side character is NOT "equal" (!=), false        
                ...
                deemed "NOT equal".
*/
bool Character::operator!= (const Character& new_character) const {  // checks if the private member variables are not equal (if two characters are not equal to each other)
    if (name_ != new_character.name_ || race_ != new_character.race_ || level_ != new_character.level_ || enemy_ != new_character.isEnemy()) {
        return true;
    }
    return false;
...
void Character::display() const {
    std::string new_enemy = "";

    if (enemy_ == true) {  // checks if the private member variable enemy_ is true
        new_enemy = "They are an enemy ";  // if it is, set the new variable "new_enemy" to the string "They are an enemy"
    } else {
        new_enemy = "They are not an enemy ";  // otherwise, set it equal to the string "They are not an enemy"
    }
