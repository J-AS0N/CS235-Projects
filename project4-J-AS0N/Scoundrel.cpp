...
void Scoundrel::display() const {
    std::string scoundrel_enemy = "";  // empty stirng
    std::string scoundrel_disguise = "";  // emtpy string

    if (isEnemy() == true) {  // if it's an enemy
        scoundrel_enemy = "an enemy.";  // set the empty string to "an enemy."
    } else {  // if it's not an enemy
        scoundrel_enemy = "not an enemy.";  // set the empty string to "not an enemy."
    }

    if (hasDisguise() == 1) {  // if hasDisguise returns true/1
        scoundrel_disguise = "TRUE";  // set the empty string to "TRUE"
    } else {  // if it's not true
        scoundrel_disguise = "FALSE";  // set the empty string to "FALSE"
    }
...
void Scoundrel::eatTaintedStew() {
    if (getRace() == "UNDEAD") {  // if the race is UNDEAD
        setVitality(getVitality() + 3);  // add 3 extra vitality to the current vitality and set it
    } else {  // if the race is NOT UNDEAD
        setVitality(1);  // set the vitality to 1
        if (getFaction() == "CUTPURSE") {  // if the race is not UNDEAD AND the getFaction() is equal to "CUTPURSE"
            setVitality(getVitality() + 3);  // add 3 extra vitality to current vitality level and set it
        } else if (getFaction() == "SILVERTONGUE") {  // if the race is not UNDEAD AND getFaction() is equal to "SILVERTONGUE"
            if (rand() % 100 < 70) {  // 70% chance of recovering 4 vitality points and 30% of resetting vitality 1
                setVitality(getVitality() + 4);  // add extra 4 vitality to current vitality level and set it
            } else {
                setVitality(1);  // set the vitality to 1
                setDagger("WOOD");  // set the dagger to WOOD
            }
        }
    }
}
